...
- Reward: episode-aware normalized 3-part objective: 50% LPM level+delta, 35% running volatility, 15% cumulative decision cost.
...
def parse_args() -> argparse.Namespace:
    ...
    # New normalized reward weights. They are automatically normalized inside the env.
    p.add_argument("--reward-weight-lpm", type=float, default=0.50)
    p.add_argument("--reward-weight-volatility", type=float, default=0.35)
    p.add_argument("--reward-weight-decision-cost", type=float, default=0.15)
    p.add_argument(
        "--lpm-target",
        type=float,
        default=0.0,
        help="Target threshold for cumulative-return LPM inside the reward.",
    )

    # Legacy arguments retained for compatibility with older commands/configs.
    # They are not used by the new default reward in SACPortfolioLPMEnv.
...
def _train_eval_one_window_worker(task: Dict[str, object]) -> Dict[str, object]:
    ...
    try:
        ...
        cfg = SACPortfolioLPMConfig(
            ...
            reward_weight_lpm=float(task["reward_weight_lpm"]),
            reward_weight_volatility=float(task["reward_weight_volatility"]),
            reward_weight_decision_cost=float(task["reward_weight_decision_cost"]),
            lpm_target=float(task.get("lpm_target", 0.0)),
...
def main() -> None:
    ...
    cfg = SACPortfolioLPMConfig(
        ...
        reward_weight_lpm=args.reward_weight_lpm,
        reward_weight_volatility=args.reward_weight_volatility,
        reward_weight_decision_cost=args.reward_weight_decision_cost,
        lpm_target=args.lpm_target,
    ...
    if args.rolling_windows:
        ...
        w_tag = _safe_filename(
            f"wLPM{args.reward_weight_lpm}_wVOL{args.reward_weight_volatility}_wCOST{args.reward_weight_decision_cost}_tau{args.lpm_target}"
        )
        run_name = (
            f"{asset.lower()}_sac_lpm3aware_ROLL_T{args.timesteps}_"
            f"train{args.train_years}y_val{args.val_months}m_test{args.test_months}m_"
            f"{hmin_tag}_{hmax_tag}_{dh_tag}_{w_tag}"
        )
        ...
        print(
            f"reward weights:       LPM={args.reward_weight_lpm} | VOL={args.reward_weight_volatility} | COST={args.reward_weight_decision_cost}"
        )
        print(f"lpm target:           {args.lpm_target}")
        print("reward memory:        LPM=0.70*level+0.30*delta | VOL=running level | COST=cumulative decision cost")
                ...
                json.dump(
                    {
                        ...
                        "reward_weight_lpm": float(args.reward_weight_lpm),
                        "reward_weight_volatility": float(args.reward_weight_volatility),
                        "reward_weight_decision_cost": float(args.reward_weight_decision_cost),
                        "lpm_target": float(args.lpm_target),
                        "reward_lpm_memory": "0.70*episode_lpm + 0.30*delta_lpm",
                        "reward_volatility_memory": "running_sum(portfolio_return^2)",
                        "reward_cost_memory": "cumulative_decision_cost_norm",
                        "warm_start": False,
        ...
        # Sequential rolling-window execution. This branch is used when
        # --parallel-windows 1. It is also the only branch compatible with
        # --warm-start because each window can reuse the previous window's model.
        w = 0
        train_start = min_start
        rolling_start_time = time.time()
        possible_windows = max(1, int(math.ceil(max(0, max_end - min_start - train_days - val_days) / max(1, step_days))))
        if int(args.max_windows) > 0:
            possible_windows = min(possible_windows, int(args.max_windows))
        ...
        while True:
            ...
            train_meta = _filter_meta_by_start_idx(train_meta_all, train_start, train_end)
            val_meta = _filter_meta_by_start_idx(train_meta_all, val_start, val_end)
            test_meta = _filter_meta_by_start_idx(eval_meta_all, test_start, test_end)

            candidate_row = {
                "candidate_index": int(len(window_candidate_rows)),
                "accepted_window": None,
            ...
            if candidate_row["status"] != "accepted":
                print(
                    f"[skip candidate {candidate_row['candidate_index']:02d}] "
                    f"train={candidate_row['train_n']} val={candidate_row['val_n']} test={candidate_row['test_n']} "
                    f"idx={train_start}:{train_end}|{val_start}:{val_end}|{test_start}:{test_end}",
                    flush=True,
                )
                train_start += step_days
                continue

            window_candidate_rows[-1]["accepted_window"] = int(w)
            wdir = out_dir / f"window_{w:02d}"
            wdir.mkdir(parents=True, exist_ok=True)
            print(
                f"\n[window {w:02d}] train={len(train_meta)} val={len(val_meta)} test={len(test_meta)} "
                f"idx={train_start}:{train_end}|{val_start}:{val_end}|{test_start}:{test_end}",
                flush=True,
            )
            _progress_line(
                ...
                possible_windows,
                rolling_start_time,
                extra=f"starting window={w:02d}",
            )
            window_start_time = time.time()
            print(f"[window {w:02d}] phase=train | timesteps={int(args.timesteps):,}", flush=True)

            train_scenarios = scenario_meta_to_env_scenarios(train_meta)
            val_scenarios = scenario_meta_to_env_scenarios(val_meta)
            test_scenarios = scenario_meta_to_env_scenarios(test_meta)

            env = build_env(pre, train_scenarios, cfg, seed=args.seed + w * 1009)
            net_arch = [int(x.strip()) for x in args.net_arch.split(",") if x.strip()]
            if args.warm_start and warm_model is not None:
                print(f"[window {w:02d}] warm-start: reusing previous window SAC weights", flush=True)
                model = warm_model
                model.set_env(env)
                if not args.keep_replay_buffer_on_warm_start:
                    reset_sac_replay_buffer(model)
            else:
                model = SAC(
                    "MlpPolicy",
                    env,
            ...
            model.save(wdir / "model.zip")
            warm_model = model
            print(f"[window {w:02d}] phase=train done | elapsed={_fmt_seconds(time.time() - window_start_time)}", flush=True)

            print(f"[window {w:02d}] phase=validation evaluation | scenarios={len(val_scenarios):,}", flush=True)
            val_log = run_policy_on_scenarios(
                model,
                pre,
                val_scenarios,
            ...
            print(f"[window {w:02d}] phase=test evaluation | scenarios={len(test_scenarios):,}", flush=True)
            test_log = run_policy_on_scenarios(
                model,
                pre,
                test_scenarios,
            ...
            print(f"[window {w:02d}] phase=save window files", flush=True)
            val_log.to_csv(wdir / "val_daily_log.csv", index=False)
            test_log.to_csv(wdir / "test_daily_log.csv", index=False)
            val_summary.to_csv(wdir / "val_episode_summary.csv", index=False)
            test_summary.to_csv(wdir / "test_episode_summary.csv", index=False)
            ...
            try:
                ...
                print(f"[WARN] parquet write failed in window {w}: {exc}", flush=True)

            print(f"[window {w:02d}] phase=charts | fraction={float(args.plot_fraction):.2%}", flush=True)
            made_charts = plot_fraction_of_test_episodes(
                test_log,
                out_dir=out_dir,
                asset=asset,
                window=w,
                fraction=float(args.plot_fraction),
                max_plots=int(args.max_plots_per_window),
            )

            print_metric_block(val_summary, f"validation window {w:02d}")
            print_metric_block(test_summary, f"test window {w:02d}")
            print(f"[window {w:02d}] charts saved: {len(made_charts)}")
            print(f"[window {w:02d}] done | elapsed={_fmt_seconds(time.time() - window_start_time)}", flush=True)

            all_val_logs.append(val_log)
            all_test_logs.append(test_log)
            all_val_summaries.append(val_summary)
            all_test_summaries.append(test_summary)
            all_report_rows.append(report_rows)
            window_rows.append({
                "window": int(w),
                "train_start_idx": int(train_start),
                "train_end_idx": int(train_end),
            ...
            _progress_line(
                ...
                possible_windows,
                rolling_start_time,
                extra="completed window",
            )
            if int(args.max_windows) > 0 and w >= int(args.max_windows):
                break
